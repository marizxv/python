...
def generate_transport_data(num_entries):
    ...
    for i in range(num_entries):
        ...
        manufacturing_date = generate_random_date(1970, 2020)
        registration_date = generate_random_date(2020, 2024)
        tech_in_dates = [generate_random_date(2020, 2024) for _ in range(random.randint(3, 6))]
...
with open('automobile.json', 'w') as file_data:
    json.dump(automobile_data, file_data, indent=4)

with open('truck.json', 'w') as file_data:
    json.dump(truck_data, file_data, indent=4)

with open('ship.json', 'w') as file_data:
    json.dump(ship_data, file_data, indent=4)

with open('plane.json', 'w') as file_data:
    json.dump(aviation_data, file_data, indent=4)

with open('bicycle.json', 'w') as file_data:
    json.dump(bicycle_data, file_data, indent=4)
